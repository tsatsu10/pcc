...
const bodySchema = z.object({
  ...
  monthlyWins: z.string().optional().default(""),
  focusNextMonth: z.string().optional().default(""),
});
...
export async function POST(req: NextRequest) {
  ...
  const lastMonthly = await prisma.review.findFirst({
    where: { userId: session.user.id, type: "monthly" },
    orderBy: { periodEnd: "desc" },
  });
  const thirtyDaysAgo = new Date(today);
  thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
  thirtyDaysAgo.setHours(0, 0, 0, 0);
  if (lastMonthly && lastMonthly.periodEnd >= thirtyDaysAgo)
    return NextResponse.json(
      { error: "A monthly review was already submitted within the last 30 days." },
      { status: 400 }
    );
  ...
  const content = {
    ...
    monthlyWins: parsed.data.monthlyWins,
    focusNextMonth: parsed.data.focusNextMonth,
  };
